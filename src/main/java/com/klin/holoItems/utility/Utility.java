...
//    public static final Set<Material> swords = Set.of(DIAMOND_SWORD, GOLDEN_SWORD, IRON_SWORD, STONE_SWORD, NETHERITE_SWORD, WOODEN_SWORD);
    ...
    /**
     * Whether food is "poor" or not is described in the <a href="https://minecraft.fandom.com/wiki/Food">wiki-article</a>
     * In summary, if saturation/foodValue = 0.2, it's poor. If it's 0.6, it's low. Etc.
     * Every food item fits into one of these categories, except technically those which provide hunger or saturation
     */
    public static final Material[] poorFoods = new ArrayList<Material>(){{
        for(Material mat : nutritionValues.keySet()){
            if(mat == SPIDER_EYE || mat == ROTTEN_FLESH){
                // These are even worse than the poor foods.
                continue;
            }

            float[] nut = nutritionValues.get(mat);
            float ratio = nut[1]/nut[0];
            // Leaving some buffer because I'm paranoid of floating-point.
            if(ratio > 0.1){
                add(mat);
            }
        }
    }}.toArray(new Material[0]);
    public static final Material[] lowFoods = new ArrayList<Material>(){{
        for(Material mat : nutritionValues.keySet()){
            if(mat == SPIDER_EYE || mat == ROTTEN_FLESH){
                // These are even worse than the poor foods.
                continue;
    ...
    public static final Material[] normalFoods = new ArrayList<Material>(){{
        for(Material mat : nutritionValues.keySet()){
            if(mat == SPIDER_EYE || mat == ROTTEN_FLESH){
                // These are even worse than the poor foods.
                continue;
    ...
    public static final Material[] goodFoods = new ArrayList<Material>(){{
        for(Material mat : nutritionValues.keySet()){
            if(mat == SPIDER_EYE || mat == ROTTEN_FLESH){
                // These are even worse than the poor foods.
                continue;
    ...
    public static final Material[] superFoods = new ArrayList<Material>(){{
        for(Material mat : nutritionValues.keySet()){
            if(mat == SPIDER_EYE || mat == ROTTEN_FLESH){
                // These are even worse than the poor foods.
                continue;
