...
public class Payload implements CommandExecutor{
    //payload buildteam 599 56 307 597 69 416 597 70 417 SOUTH
    //payload world -2 67 177 -1 69 205 -1 73 206 SOUTH
    @Override
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if(sender instanceof Player && !sender.isOp())
            return true;

        if(cmd.getName().equals("spawn") && args.length>=4){
            spawn(0, Bukkit.getWorld(args[0]), Integer.parseInt(args[1]), Integer.parseInt(args[2]), Integer.parseInt(args[3]), BlockFace.UP);
            return true;
        }

        if (!cmd.getName().equals("payload"))
            return true;
        if(args.length<11)
            return true;
    ...
    private AbstractMap.SimpleEntry<Set<FallingBlock>, FallingBlock> spawn(int index, World world, int x, int y, int z, BlockFace direction){
